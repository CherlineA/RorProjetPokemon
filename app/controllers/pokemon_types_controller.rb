class PokemonTypesController < ApplicationController
  before_action :set_pokemon_type, only: %i[ show edit update destroy ]

  # GET /pokemon_types or /pokemon_types.json
  def index
    @pokemon_types = PokemonType.all
  end

  # GET /pokemon_types/1 or /pokemon_types/1.json
  def show
  end
  ...
  # POST /pokemon_types or /pokemon_types.json
  def create
  ...
  # PATCH/PUT /pokemon_types/1 or /pokemon_types/1.json
  def update
    respond_to do |format|
  ...
  # DELETE /pokemon_types/1 or /pokemon_types/1.json
  def destroy
  ...
  private
    ...
    def pokemon_type_params
      params.require(:pokemon_type).permit(:nomType)
    end
end
